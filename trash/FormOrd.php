...
<script type="text/javascript">
    ...
    function createOrderElem() {
        const elemRow = document.createElement("div"),
            descriptionDiv = document.createElement("div"),
            descriptionLable = document.createElement("label"),
            descriptionTextarea = document.createElement("textarea"),
            photoDiv = document.createElement("div"),
            photoLable = document.createElement("label"),
            photoInput = document.createElement("input");

        elemRow.className = "row";
        descriptionDiv.className = "col-12 col-sm-6 col-md-8";
        descriptionLable.innerHTML = "Описание";
        descriptionLable.htmlFor  = "name";
        descriptionTextarea.className = "form-control";
        descriptionTextarea.ariaLabel = "With textarea";
        photoDiv.className = "col-6 col-md-4";
        photoLable.innerHTML = "Прикрепить Фотографии";
        photoLable.htmlFor  = "name";
        photoInput.classList.add("file");
        photoInput.type = "file";

        descriptionDiv.appendChild(descriptionLable);
        descriptionDiv.appendChild(descriptionTextarea);
        photoDiv.appendChild(photoLable);
        photoDiv.appendChild(photoInput);
        elemRow.appendChild(descriptionDiv);
        elemRow.appendChild(photoDiv);

        return elemRow;
    }

    const orderNumberSelector = document.querySelector(".sum_of_orders");
    orderNumberSelector.addEventListener('change', (event) => {
        const newOrderList = [];
            orderListElem = document.querySelector("#order-list"),
            orderNumber = Number(event.currentTarget.value);

        for (let i = 0; i < orderNumber; i++)
            newOrderList.push(createOrderElem());

        orderListElem.replaceChildren(...newOrderList);
    });
